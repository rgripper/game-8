...
function connectToServer(socket, authToken) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield rxjs_1.firstValueFrom((socket.isOpen() ? rxjs_1.from([true]) : rxjs_1.fromEvent(socket, 'open')).pipe(operators_1.timeout({ each: 1000, with: () => rxjs_1.throwError(new Error('Timed out waiting for socket to open')) }), operators_1.tap(() => socket.send(control_commands_1.AuthorizationPrefix + authToken)), operators_1.mergeMap(() => rxjs_1.fromEvent(socket, 'message').pipe(operators_1.first(), operators_1.timeout({
            each: 1000,
