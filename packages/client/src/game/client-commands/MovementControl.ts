...
function reduceTrackerWithKey(
    movementKeys: MovementKeys,
    movementTracker: MovementTracker,
    key: string,
    axisStateTrackerReducer: AxisTrackerReducer,
): MovementTracker {
    switch (key) {
        case movementKeys.back:
            return {
                ...movementTracker,
                vertical: axisStateTrackerReducer(movementTracker.vertical, AxisState.Positive),
            };
        case movementKeys.forward:
            return {
                ...movementTracker,
                vertical: axisStateTrackerReducer(movementTracker.vertical, AxisState.Negative),
            };
        case movementKeys.right:
            return {
                ...movementTracker,
                horizontal: axisStateTrackerReducer(movementTracker.horizontal, AxisState.Positive),
            };
        case movementKeys.left:
            return {
                ...movementTracker,
                horizontal: axisStateTrackerReducer(movementTracker.horizontal, AxisState.Negative),
            };
        default:
...
function getDirection(movementTracker: MovementTracker): number {
    ...
    // top right
    if (
        movementTracker.vertical.active === AxisState.Positive &&
        movementTracker.horizontal.active === AxisState.Positive
    ) {
        return 45;
    }
    ...
    if (
        movementTracker.vertical.active === AxisState.Positive &&
        movementTracker.horizontal.active === AxisState.Negative
    ) {
        return 135;
    }
    ...
    // bottom left
    if (
        movementTracker.vertical.active === AxisState.Negative &&
        movementTracker.horizontal.active === AxisState.Negative
    ) {
        return 225;
    }
